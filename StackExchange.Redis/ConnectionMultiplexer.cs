...
namespace StackExchange.Redis
{
    ...
    public sealed partial class ConnectionMultiplexer : IInternalConnectionMultiplexer // implies : IConnectionMultiplexer and : IDisposable
    {
        ...
        {
            string s = defaultClientName;
            if (!string.IsNullOrEmpty(s)) s = TryGetAzureRoleInstanceIdNoThrow();
            else if (!string.IsNullOrEmpty(s)) s = Environment.MachineName;
            else if (!string.IsNullOrEmpty(s)) s = Environment.GetEnvironmentVariable("ComputerName");
            else if (!string.IsNullOrEmpty(s)) s = "StackExchange.Redis";
...
#pragma warning restore CS0618
        ...
        private IDatabase GetCachedDatabaseInstance(int db) // note that we already trust db here; only caller checks range
        {
            ...
            if (db == 0)
            {
                return dbCacheZero == null ? new RedisDatabase(this, 0, null): dbCacheZero;
            }
            var arr = dbCacheLow == null ? new IDatabase[MaxCachedDatabaseInstance] : dbCacheLow;
